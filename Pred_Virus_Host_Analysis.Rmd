...
summaryData <- summaryData %>% mutate(protein_descriptions = str_replace_all(protein_descriptions, ",", ":"))

summaryData <- summaryData %>% mutate_all(as.character) %>% mutate_if(is.character, str_replace_all, pattern = ",", replacement =":")

save(summaryData, file = "~/bin/PredVirusHost/archaeal_hmm_accessions_descriptions.Rda")
write.csv(summaryData, file = "~/bin/PredVirusHost/archaeal_hmm_accessions_descriptions.csv", row.names = F, quote = F)
...
```{r eukaryotic_proteins_and_hmms}
file_path <- "~/PredVirusHost/update_march_2020/"

proteins <- read.table(paste(file_path, "eukaryotic_proteins_and_functions.txt", sep = ""), sep = "\t", header = F, as.is = T, quote = "", comment.char = "")
...
head(proteins)
colnames(proteins) <- c("protein_accession",  "genome", "protein_description")

hmms <- read.table(paste(file_path, "euVOG_protein_hmm_list.txt", sep = ""), sep = "\t", header = F, as.is = T)

head(hmms)
colnames(hmms) <- c("protein_accession", "genome_2", "hmm_accession")
...
summaryData <- summaryData %>% select(-genera)

save(summaryData, file = "~/bin/PredVirusHost/eukaryotic_hmm_accessions_descriptions.Rda")
write.csv(summaryData, file = "~/bin/PredVirusHost/eukaryotic_hmm_accessions_descriptions.csv", row.names = F, quote = F)
...
```{r sens_and_spec}
file_path <- "~/phd/PredVirusHost//update_march_2020/"
eukaryoticRes <- read.table(file = paste(file_path, "/eukaryotic_simulation.txt", sep = ""), sep = "\t", header = T)
archaealRes <- read.table(file = paste(file_path, "/archaeal_simulation.txt", sep = ""), sep = "\t", header = T)
phagelRes <- read.table(file = paste(file_path, "/phage_simulation.txt", sep = ""), sep = "\t", header = T)
...
proteinAcc <- data.frame(protein_count = 0, accuracy = 0, count = 0)
...
proteinAcc <- proteinAcc%>%bind_rows(data.frame(protein_count = i, accuracy = acc, count = nrow(tmp%>%filter(!is.na(call)))))
...
proteinAcc <- proteinAcc%>%mutate(accuracy = ifelse(is.na(accuracy), 100, accuracy))
...
ggsave("~/phd/PredVirusHost/figures/sensitivity_specificity_proteins.svg", plot = p, device = "svg")

p <- ggplot() +
  geom_line(data = proteinAcc, aes(x = protein_count, y = accuracy), size = 1.5) +
  coord_cartesian(ylim = c(0, 105), xlim = c(0, 50)) +
  labs(x = "Number of Proteins", y = "Percentage (Accuracy)") +
  theme_bw()
p
...
p <- ggplot() +
  geom_histogram(data = all %>% filter(archaeal_score > 0), aes(x = archaeal_score, y = ..count../max(..count..)*100), fill='grey', binwidth = 10) +
  ...
  coord_cartesian(ylim = c(0, 105), xlim = c(0, 300)) +
  labs(x = "Score", y = "Percentage (Sensitivity and Specificity)") +
  theme_bw()

p
ggsave("~/phd/PredVirusHost/figures//sensitivity_specificity_scores.svg", plot = p, device = "svg")
...
metagenomes <- metagenomes %>% mutate(difference.percentage = ifelse(is.na(difference.percentage), 0, ifelse(is.infinite(difference.percentage), 100, difference.percentage)))

hotpools <- hotpools %>% mutate(difference.percentage = abs(difference.percentage))
...
hotpools <- hotpools %>% mutate(difference.percentage = ifelse(is.na(difference.percentage), 0, ifelse(is.infinite(difference.percentage), 100, difference.percentage)))

hotpools <- hotpools %>% mutate(difference.percentage = abs(difference.percentage))
                                            

metagenomes$difference.percentage[metagenomes$call == "none"] <- 0                                               
hotpools$difference.percentage[hotpools$call == "none"] <- 0                                               
                                           

ggplot() +
  geom_freqpoly(data = metagenomes, aes(x = protein.counts), binwidth = 1, colour = "red") +
  geom_freqpoly(data = hotpools, aes(x = protein.counts), binwidth = 1, colour = "blue") +
  xlim(c(0,20))

metagenomes$group <- "Metagenomes"
hotpools$group <- "Thermal"
metagenomes$group[metagenomes$protein.counts >= 3] <- "3 or more"
metagenomes$group[metagenomes$protein.counts < 3] <- "less than 3"
hotpools$group[hotpools$protein.counts >= 3] <- "3 or more"
hotpools$group[hotpools$protein.counts < 3] <- "less than 3"


all <- metagenomes %>% bind_rows(hotpools)

ggplot() + 
  geom_bar(data = metagenomes, aes(x = call, y = ..count.., group = group, fill = group), position = "dodge")

ggplot() + 
  geom_bar(data = hotpools %>% filter(call != "none"), aes(x = call, y = ..count.., group = group, fill = group), position = "dodge")


p <- ggplot() +
  geom_bar(data = metagenomes, aes(x = call, y = ..count..))
p

ggsave("~/metagenomes_bargraph.svg", plot = p, device = "svg")

p <-ggplot() +
  geom_bar(data = hotpools %>% filter(call != "none"), aes(x = call, y = ..count..))
p

ggsave("~/thermal_bargraph.svg", plot = p, device = "svg")

```

```{r plot_genomes}
gff <- read.table("~/bin/PredVirusHost/predvirushost_test_files/test_2.tmp.folder/Acidianus_bottle-shaped_virus_complete_genome.]_contig.gff", sep = "\t", fill = T, comment.char = "#", quote = "")
...
ends1 <- gff$end
strands1 <- gff$strand
gff <- gff %>% mutate(colours = ifelse(call == "archaeal", "blue", ifelse(call == "phage", "red", ifelse(call == "eukaryotic", "green", "black"))))
cols1 <-gff$colours
